...
def filter_candidates(answers_list, all_characters):
    if not answers_list:
        return [(name, data, 0) for name, data in list(all_characters.items())[:20]]

    user_age_group = None
    if answers_list:
        user_age_group = extract_age_group(dict(answers_list))

    # Convert answers to dict for fast lookup
    answers_dict = dict(answers_list)

    remaining = list(all_characters.items())
    for name, data in remaining[:]:  # iterate over a copy
        char_answers = data.get("answers", {})

        # --- STATUS FILTERING: Exclude mismatched statuses ---
        eliminated = False
    ...
    # --- Now apply answer-by-answer filtering (your original logic) ---
    for question, user_answer in answers_list:
        norm_q = normalize_key(question)
        new_remaining = []
        for name, data in remaining:
            # Age group filter (already applied globally)
            ...
            for cq, ca in char_answers.items():
                if normalize_key(cq) == norm_q:
                    ...
                    break
            if not matched:
                keep = (user_answer != "yes")  # safe to keep if question not in char data
            if keep:
...
def get_active_questions(questions_schema, all_characters, answers_list, role, question_group_key):
    ...
    # Get current candidate names (once!)
    ...
    for q in base_questions:
        ...
        if names_with_q & candidate_names:  # fast set intersection
...
def get_next_question(answers_list, asked_set, questions_schema, all_characters):
    ...
    if status == "active":
        ...
        # >>> NEW: National team manager support <<<
        if role == "manager":
            # Only proceed if birth country is confirmed
            if any(answers_dict.get(q) == "yes" for q in active_origin_country_qs):
                # Ask if managing a national team
                nat_team_qs = questions_schema.get("manager_national_team", [])
                if nat_team_qs and not any(answers_dict.get(q) == "yes" for q in nat_team_qs):
                    unasked_nat = [q for q in nat_team_qs if q not in answers_dict]
                    if unasked_nat:
                        return random.choice(unasked_nat)

                # If YES to national team, ask continent and country
                if answers_dict.get("Is this manager currently coaching a national team?") == "yes":
                    # Ask continent
                    nat_cont_qs = questions_schema.get("manager_national_continent", [])
                    if nat_cont_qs and not any(answers_dict.get(q) == "yes" for q in nat_cont_qs):
                        unasked_cont = [q for q in nat_cont_qs if q not in answers_dict]
                        if unasked_cont:
                            return random.choice(unasked_cont)

                    # Infer national continent
                    nat_continent = None
                    for q in nat_cont_qs:
                        if answers_dict.get(q) == "yes":
                            match = re.search(r"coaching a ([a-zA-Z\s]+) national team", q)
                            if match:
                                nat_continent = normalize_continent_name(match.group(1))
                                break

                    if nat_continent:
                        # Ask national country (continent-specific)
                        nat_country_key = f"manager_national_country_{nat_continent}"
                        nat_country_qs = questions_schema.get(nat_country_key, [])
                        if nat_country_qs and not any(answers_dict.get(q) == "yes" for q in nat_country_qs):
                            for q in nat_country_qs:
                                if q not in answers_dict:
                                    return q

                    # Prevent club questions for national team managers
                    return None

        # Work continent (only if NOT national team manager)
        if role == "manager" and answers_dict.get("Is this manager currently coaching a national team?") != "yes":
            work_continent = None
        ...
        # For players, keep original work logic
        elif role == "player":
            # Work continent
            work_continent = None
            work_continent_key = f"{role}_league_continent"
...
def question():
    ...
    # In /question route, after loading all_chars:
    global QUESTION_TO_NAMES
    if not QUESTION_TO_NAMES:  # Build once
...
def answer():
    ...
    if request.method == "POST":
        ...
        if action == "correct":
            return render_template("answer.html", guess=f"Yes! It's {best_name}!", show_success=True, play_again=True)
        elif action == "wrong":
...
def learn():
    if request.method == "POST":
        ...
        # ✅ Check if description is provided (now compulsory)
        if not description:
            return render_template(
                "learn.html",
        ...
        # ✅ Profanity check using BOTH methods for maximum coverage
        # Check with better_profanity library
        ...
        # Check with custom profanity list (for additional coverage)
